...
def admin_dashboard_data():
    ...
    try:
        ...
        for record in result:
            ...
            vendor_risks.append({
                'vendor_gstin': gstin,
                'vendor_name': f"Entity {gstin[-4:]}",  # Use last 4 chars for readability
        ...
        # Get fraud pattern counts and details from graph analysis
        ...
        # 3. Spider Web: Taxpayers with shared contacts (suspicious networks)
        spider_query = """
        ...
        return jsonify({
            ...
            'vendor_risks': vendor_risks[:100],  # Limit to first 100 for performance
            'patterns': {
...
def business_owner_dashboard_data(gstin):
    ...
    try:
        ...
        # Calculate risk score (same logic as admin dashboard)
        risk_score = 0.3
        ...
        # Check for shared contact networks
        spider_query = """
        ...
        # Get vendor risks (partners with their risk levels)
        vendor_query = """
...
{f'  Partners in circular trade: {", ".join(circular_partners[:5])}' if circular_partners else ''}
...
RECOMMENDATIONS:
{f'- URGENT: You are involved in {circular_count} circular trade patterns. This is a serious fraud indicator.' if circular_count > 0 else ''}
{f'- WARNING: {ghost_count} invoices lack e-way bill backing. Ensure all invoices have proper documentation.' if ghost_count > 5 else ''}
{f'- ALERT: {shared_contacts} entities share contact information with you. This may indicate a fraud network.' if shared_contacts > 10 else ''}
...
def graph(current_user):
    """
    GET /graph - Return graph data from Neo4j database
    
    Fetches nodes and relationships from Neo4j with RBAC filtering.
    For admins: returns all data
    For business owners: returns only their connected subgraph
    """
    try:
        from utils.db_connection import get_neo4j_connection
        ...
        try:
            # Build Cypher query based on user role
            if current_user.role == 'Admin':
                # Admin: Get all nodes and relationships in a single query to ensure consistency
                combined_query = """
                ...
                // Collect all unique nodes from these relationships
                ...
                # Now get edges, but only for nodes we have
                edges_query = """
                MATCH (source)-[r]->(target)
                WHERE (source:Taxpayer OR source:Invoice OR source:EwayBill)
                  AND (target:Taxpayer OR target:Invoice OR target:EwayBill)
                ...
                # Business Owner: Get only their subgraph (2-hop neighborhood)
...
def risk_details(current_user, gstin):
    ...
    try:
        ...
        # Check for shared contact networks (spider web)
        spider_query = """
        MATCH (t:Taxpayer {gstin: $gstin})-[:SHARED_CONTACT]->(other:Taxpayer)
        RETURN count(DISTINCT other) as shared_contacts
        """
        ...
        # Generate top drivers based on actual data
        top_drivers = [
            {
...
Risk Drivers:
1. KYC Score contributes {kyc_factor * 0.4:.2%} to overall risk
2. Transaction patterns contribute {0.2 if transaction_count == 0 or transaction_count > 50 else 0.0:.2%} to overall risk
3. Partner network contributes {0.1 if unique_partners > 20 else 0.0:.2%} to overall risk
